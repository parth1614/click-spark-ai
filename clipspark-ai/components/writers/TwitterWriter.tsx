...
export default function TwitterWriter() {
  ...
  const fetchSaved = async () => {
    ...
    try {
      const res = await fetch("/api/content/saved?type=twitter");
      const data = await res.json();
      setSaved(data.items ?? []);
    } catch (e) {
      console.error(e);
  ...
  const renderThreads = (threadList: Thread[]) =>
    threadList.map((thread, i) => (
      <ContentCard key={i} label={`Thread ${i + 1}`} labelColor="text-sky-600">
        <div className="flex justify-end mb-2">
          <CopyButton text={[thread.hook, ...thread.tweets].join("\n\n")} />
        </div>
        <p className="text-gray-900 text-sm font-medium mb-3">{thread.hook}</p>
  ...
  return (
    <div>
      <SectionHeader
        ...
        description="Generate viral-worthy tweet threads with AI-generated visuals."
      />
          ...
          <div className="mt-6 space-y-4">
            ...
            {loading && (
              <div className="flex items-center justify-center py-12 text-gray-600">
                ...
                Generating threads + visual...
              </div>
            )}
            {!loading && renderThreads(threads)}
          </div>
        </>
      )}
      ...
      {tab === "saved" && (
        <div className="space-y-4">
          ...
          {!loadingSaved && saved.length === 0 && (
            <EmptyState
              icon={<MessageCircle className="w-10 h-10 text-gray-500" />}
              message="No saved Twitter threads yet"
            />
          )}
          {!loadingSaved &&
            saved.map((item) => (
              ...
              >
                ...
                <div className="p-4 space-y-3">
                  {renderThreads(item.content.threads)}
                </div>
              </div>
            ))}
        </div>
